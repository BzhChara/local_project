...
# 定义寡糖编码数据Embedding模型
class GlycanEncode(nn.Module):
    def __init__(self, input_dim, embedding_dim):
        super(GlycanEncode, self).__init__()
        self.embedding = nn.Embedding(input_dim, embedding_dim)
        self.fc = nn.Linear(embedding_dim, 32)

    def forward(self, x):
        embedding_vector = self.embedding(x)
        glycan = self.fc(embedding_vector.mean(dim=1))  # 取平均值以匹配全连接层的输入形状
        return glycan
...
# 初始化Embedding模型
glycan_embedding = GlycanEncode(input_dim=176, embedding_dim=64).to(device)
...
def train_model(model, epochs):
    ...
    for epoch in range(epochs):
        for i in range(0, train_size, batch_size):  # 只遍历前80%的数据
            ...
            # Embedding寡糖编码
            glycan_encode = glycan_embedding(glycan_data)
            # 合并特征向量
            combined_features = torch.cat((protein_features, glycan_encode, values_data), dim=1)
