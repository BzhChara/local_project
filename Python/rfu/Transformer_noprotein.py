...
# 初始化Embedding模型
protein_embedding = ProteinEmbedding(input_dim=2710, embedding_dim=64).to(device)
...
def train_model(model, epochs):
    ...
    for epoch in range(epochs):
        for i in range(0, train_size, batch_size):  # 只遍历前80%的数据
            ...
            # Embedding蛋白质编码
            protein_encode = protein_embedding(protein_data)
            # 提取寡糖特征
            glycan_features = glycan_extractor(glycan_data)
            # 合并特征向量
            combined_features = torch.cat((protein_encode, glycan_features, values_data), dim=1)
