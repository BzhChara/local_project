...
# 定义目标变量列名
protein_cols = data.columns[:1280]        # 前1280列为蛋白质特征
# oligo_cols = data.columns[1280:1280+342]  # 接下来的342列为寡糖特征
y_col = data.columns[-1]                  # 最后一列为目标变量RFU
...
# 替换原有的RFU_scaled列
train_final = train[protein_cols].cbind(
    h2o.H2OFrame(train_rfu_quantile, column_names=["RFU_scaled"])
)
test_final = test[protein_cols].cbind(
